...
We tuned 50 values of Complexity Parameters (cp, the minimum improvement in the model needed at each node) from 𝑒−6 to 𝑒−3. The model with the slargest cross-validation accuracy is with cp `r tree1$bestTune`.
...
If the st_slope is flat and the patient is male, the probability of having heart disease is 47%.

If the st_slope is up and the patient has the Asymptomatic type of chest pain, the probability of not having heart disease is 28%.
...
In the MARS model, we tuned the maximum degree of interaction (degree) from 1 to 4 and the maximum number of terms (including intercept) in the pruned model (nprune) from 5 to 20. The best model with the smallest cross-validation RMSE is with nprune = 11 and degree = 1. The best model selected 11 of 19 terms, and 8 of 15 predictors. 
...
```{r}
p1 <- pdp::partial(model.mars, pred.var = c("cholesterol"), grid.resolution = 10) %>% autoplot()

p2 <- pdp::partial(model.mars, pred.var = c("cholesterol", "oldpeak"),
grid.resolution = 10) %>%
...
To examine the marginal effects of predictors, we created a partial dependence plot for cholesterol and also an interaction PD plot for both cholesterol and oldpeak.
